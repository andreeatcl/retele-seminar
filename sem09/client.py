...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE LOCALA: Nu esti conectat. Foloseste CONNECT.")
            continue
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE LOCALA: PUBLISH necesita un text de mesaj.")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if not este_conectat:
            ...
            continue
        if len(parti) < 2:
            print("EROARE LOCALA: DELETE necesita un ID numeric.")
            continue
        try:
            int(parti[1].strip())
        except ValueError:
            print("EROARE LOCALA: ID invalid pentru DELETE. Trebuie numar intreg.")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
